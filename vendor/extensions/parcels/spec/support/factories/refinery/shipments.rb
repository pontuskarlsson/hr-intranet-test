...
FactoryGirl.define do
  factory :shipment, :class => Refinery::Parcels::Shipment do
    ...
    bill_to ::Refinery::Parcels::Shipment::BILL_TO.first

    # A factory that creates a Shipment with a courier that
    # can be handled by EasyPost
    factory :shipment_with_easypost do
      courier ::Refinery::Parcels::Shipment::COURIERS.detect { |_,v| v[:easypost] }[0]
    ...
    factory :shipment_with_rates do
      ...
      sequence(:rates_content) { |n| n.times.map { |n|
        { id: "rate_qwerty#{n}",
          object: 'Rate',
          mode: 'Test',
          carrier: ::Refinery::Parcels::Shipment::COURIER_DHL,
