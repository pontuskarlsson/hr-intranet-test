do ($ = jQuery, _ = @._) ->
  ...
  $ ->
    ...
    lineItemTmpl = $('.js_line_item_tmpl').html().replace(/xero_receipt\[xero_line_items_attributes]\[1]/g, ['xero_receipt[xero_line_items_attributes][', $('.receipt-line-items').children().length, ']'].join(''))
