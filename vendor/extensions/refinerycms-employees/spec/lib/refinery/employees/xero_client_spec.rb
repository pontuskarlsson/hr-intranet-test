require 'spec_helper'

module Refinery
  module Employees
    describe XeroClient do
      let(:xero_api_key_file) { FactoryBot.create(:xero_api_key_file) }
      let(:xero_client) { XeroClient.new(xero_api_key_file.organisation) }

      describe '#sync_accounts' do
        let(:xero_account) { FactoryBot.create(:xero_account, guid: "00000000-0000-0000-1234-000000000000", name: 'Travel') }
        let(:account) { Xeroizer::Record::Account.build({ account_id: "00000000-0000-0000-1234-000000000000", name: 'Office Expense', code: '1234' }, nil) }

        context 'if does not find any matching Account in Xero' do
          before { allow_any_instance_of(XeroClient).to receive(:all_accounts).and_return([]) }

          it 'makes an existing XeroAccount inactive' do
            expect(xero_account.inactive).to eq false

            expect{ xero_client.sync_accounts }.to_not raise_error
            expect(xero_account.reload.inactive).to eq true
          end
        end

        context 'for any Account in Xero that does not match an existing XeroAccount' do
          before { allow_any_instance_of(XeroClient).to receive(:all_accounts).and_return([account]) }

          it 'creates a new XeroAccount' do
            expect( XeroAccount.find_by_guid(account.account_id) ).to be_nil

            expect{ xero_client.sync_accounts }.to_not raise_error
            expect( XeroAccount.find_by_guid(account.account_id) ).to be_present
          end
        end

        context 'if there is a matching Account in Xero' do
          before { allow_any_instance_of(XeroClient).to receive(:all_accounts).and_return([account]) }

          it 'updates an existing XeroAccount' do
            expect( xero_account.guid ).to eq account.account_id
            expect( xero_account.name).to_not eq account.name

            expect{ xero_client.sync_accounts }.to_not raise_error
            expect( xero_account.reload.name ).to eq account.name
          end
        end
      end

      describe '#sync_contacts' do
        let(:xero_contact) { FactoryBot.create(:xero_contact, guid: "00000000-0000-0000-1234-000000000000", name: 'Foo Bar') }
        let(:contact) { Xeroizer::Record::Contact.build({ contact_id: "00000000-0000-0000-1234-000000000000", name: 'Jane Deer' }, nil) }

        context 'if does not find any matching Contact in Xero' do
          before { allow_any_instance_of(XeroClient).to receive(:all_contacts).and_return([]) }

          it 'makes an existing XeroContact inactive' do
            expect(xero_contact.inactive).to eq false

            expect{ xero_client.sync_contacts }.to_not raise_error
            expect(xero_contact.reload.inactive).to eq true
          end
        end

        context 'for any Contact in Xero that does not match an existing XeroContact' do
          before { allow_any_instance_of(XeroClient).to receive(:all_contacts).and_return([contact]) }

          it 'creates a new XeroContact' do
            expect( XeroContact.find_by_guid(contact.contact_id) ).to be_nil

            expect{ xero_client.sync_contacts }.to_not raise_error
            expect( XeroContact.find_by_guid(contact.contact_id) ).to be_present
          end
        end

        context 'if there is a matching Contact in Xero' do
          before { allow_any_instance_of(XeroClient).to receive(:all_contacts).and_return([contact]) }

          it 'updates an existing XeroContact' do
            expect( xero_contact.guid ).to eq contact.contact_id
            expect( xero_contact.name).to_not eq contact.name

            expect{ xero_client.sync_contacts }.to_not raise_error
            expect( xero_contact.reload.name ).to eq contact.name
          end
        end
      end

    end
  end
end
