  ...
  module Employees
    describe PublicHoliday do
      describe "validations" do
        let(:public_holiday) { FactoryBot.build(:public_holiday) }
        ...
        it 'validates holiday_date and country uniqueness' do
          ...
          FactoryBot.create(:public_holiday, holiday_date: public_holiday.holiday_date, country: public_holiday.country)
          expect(public_holiday).not_to be_valid
        end
        it 'validates event_id uniqueness if present' do
          expect(public_holiday).to be_valid

          public_holiday.event_id = FactoryBot.create(:public_holiday).event_id
          expect(public_holiday).not_to be_valid
        end
      end
    end
