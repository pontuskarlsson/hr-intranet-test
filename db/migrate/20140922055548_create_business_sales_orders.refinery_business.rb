# This migration comes from refinery_sales_orders (originally 1)
class CreateBusinessSalesOrders < ActiveRecord::Migration
  ...
  def down
    remove_index :refinery_business_sales_orders, :order_id
    remove_index :refinery_business_sales_orders, :active
    remove_index :refinery_business_sales_orders, :position

    if defined?(::Refinery::UserPlugin)
      ::Refinery::UserPlugin.destroy_all({:name => 'refinerycms-business'})
    end

    if defined?(::Refinery::Page)
      ::Refinery::Page.delete_all({:link_url => '/business/sales_orders'})
    end
